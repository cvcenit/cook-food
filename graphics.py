...
class TextButton(Button):
	...
	def is_hovered(self):
		...
		self._y <= pyxel.mouse_y <= self._y + HEADER_FONT_SIZE

	def draw_button(self):
		text(self._x, self._y, self._text, (8 if self.is_hovered() else self._color), font=HEADER_FONT)
...
class MainMenuModel:
	def __init__(self):
		self._buttons = [
		TextButton(50, 48, "Play", 1),
		TextButton(50, 96, "Settings", 1)
		]
		self._states = ["game", "settings"]
...
class MainMenuController:
	...
	def update(self):
		if not self.model.is_current_screen:
			self.switch_screen(self._model.next_state)
